...
def load_model(model):
    if model == "llava-1.5":
        model_path = os.path.expanduser("/PATH/llava-v1.5-7b")
        return load_llava_model(model_path)

    elif model == "llava-1.5-13b":
        model_path = os.path.expanduser("/PATH/llava-v1.5-13b")
        return load_llava_model(model_path)
...
def prepare_llava_inputs(template, query, image, tokenizer):
    ...
    prompt_end_idx = img_end_idx + len(token_after[0])
    image_token = (
        torch.ones([batch_size, 1], dtype=torch.int64, device="cuda")
        * IMAGE_TOKEN_INDEX
    )
    ...
    return qu, img_start_idx, img_end_idx, prompt_end_idx, kwargs
...
class ModelLoader:
    ...
    def load_model(self):
        if self.model_name == "llava-1.5":
        ...
        elif self.model_name == "llava-1.5-13b":
            model_path = os.path.expanduser("/PATH/llava-v1.5-13b")
            self.tokenizer, self.vlm_model, self.image_processor, self.llm_model = (
                load_llava_model(model_path)
            )
            ...
            yaml_path = "./modPAI/mllm/config/config.yml"
            self.tokenizer, self.vlm_model, self.image_processor, self.llm_model = (
    ...
    def prepare_inputs_for_model(self, template, query, image):
        if self.model_name == "llava-1.5" or self.model_name == "llava-1.5-13b":
            questions, img_start_idx, img_end_idx, prompt_end_idx, kwargs = prepare_llava_inputs(
            ...
            questions, img_start_idx, img_end_idx, prompt_end_idx, kwargs = prepare_shikra_inputs(
                template, query, image, self.tokenizer
            )
        else:
        ...
        self.prompt_end_idx = prompt_end_idx
    ...
    def init_cfg_processor(self, questions, gamma=1.1, beam=1, start_layer=0, end_layer=32):
        ...
        elif self.model_name == "llava-1.5" or self.model_name == "llava-1.5-13b":
