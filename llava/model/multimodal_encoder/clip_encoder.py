...
def get_cls_token(vit_attn, shared_dict, num_vision_tokens=576, local_token_ratio=0.25):
    cls_attn = vit_attn[-2][0, :, 0, -num_vision_tokens:].sum(dim=0)
    _, cls_sample_tokens = torch.topk(cls_attn, int(num_vision_tokens * 0.25), largest=True)
    shared_dict['cls_sample_tokens'] = cls_sample_tokens
    ...
    def forward(self, images):
        if type(images) is list:
            ...
            if is_internvl_14b_model(self.vision_tower_name):
                ...
                image_features = self.feature_select(image_forward_outs).to(images.dtype)
                get_cls_token(image_forward_outs.attentions, self.shared_dict)
