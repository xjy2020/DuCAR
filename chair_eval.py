...
parser.add_argument("--cls_token_ratio", type=float, default=0.25)
...
parser.add_argument("--threshold_p", type=float, default=0.85)
...
coco_loader = torch.utils.data.DataLoader(
    coco_dataset, batch_size=args.batch_size, shuffle=False, num_workers=1
)
...
file_parts = [
    ...
    f"_cls_token_ratio_{args.cls_token_ratio}" if args.cls_token_ratio else "",
    ...
    f"_threshold_{args.threshold_p}" if args.use_attn else "",
]
...
for batch_id, data in tqdm(enumerate(coco_loader), total=len(coco_loader)):
    ...
    if model_loader.model_name == "llava-1.5":
        llama_modify(
